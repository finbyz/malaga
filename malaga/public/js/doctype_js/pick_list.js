frappe.ui.form.on('Pick List', {
	...
	setup: (frm) => {
		...
		frm.set_query("sales_order", function () {
			return {
				query: "malaga.api.sales_order_query",
				filters: {
	...
	onload: function (frm) {
		// Only load SO items on brand-new docs; reloading on open marks saved docs as "Not Saved"
		if (frm.is_new() && frm.doc.sales_order && !frm.doc.sales_order_item?.length) {
	...
	get_locations: function (frm) {
		if (!frm.doc.company || (!frm.doc.sales_order && !frm.doc.customer && !frm.doc.item)) {
			return;
		}

		frappe.call({
			method: 'malaga.doc_events.pick_list.get_item_from_sales_order',
			args: {
				company: frm.doc.company,
				item_code: frm.doc.item,
				customer: frm.doc.customer,
			...
			callback: function (r) {
				if (!r.message || !r.message.length) {
					frappe.msgprint({
						...
						message: __("No pending items found for this Customer / Sales Order."),
						indicator: "orange"
					});
					return;
				}

				const default_warehouse = frm.doc.parent_warehouse;
				frm.doc.locations = [];
				r.message.forEach(function (item) {
					if ((item.so_qty - item.picked_qty) > 0.0) {
						...
						if (default_warehouse) {
							frappe.model.set_value(d.doctype, d.name, 'warehouse', default_warehouse);
						}
					}
				});
				frm.refresh_field('locations');
	...
	get_item_qty: function (frm) {
		...
		frappe.call({
			method: 'malaga.doc_events.pick_list.get_item_qty',
			args: {
				company: frm.doc.company,
				item_code: frm.doc.item,
				customer: frm.doc.customer,
	...
	get_so_items: function (frm, force) {
		if (!frm.doc.sales_order) {
			return;
		}

		if (!force && !frm.is_new() && frm.doc.sales_order_item?.length) {
			return;
		}

		frm.doc.sales_order_item = [];
		frappe.call({
			method: 'malaga.doc_events.pick_list.get_sales_order_items',
			args: {
				sales_order: frm.doc.sales_order,
			},
			callback: function (r) {
				if (r.message) {
					let seen = {};
					r.message.forEach(function (item) {
						if (seen[item.sales_order_item]) {
							return;
						}
						seen[item.sales_order_item] = 1;
						...
						frappe.model.set_value(d.doctype, d.name, 'sqf_rate', item.sqf_rate);
...
const select_items = (args) => {
	const open_dialog = (enriched) => {
		frappe.require("assets/malaga/js/utils/item_selector.js", function () {
			new ItemSelector(enriched);
		});
	};

	const needs_so = args.sales_order && (!args.customer || !args.date || !args.so_qty);
	const needs_soi = args.sales_order_item && !args.so_qty;

	if (!needs_so && !needs_soi) {
		open_dialog({
			...args,
			customer: args.customer || args.frm?.doc?.customer,
			date: args.date || args.transaction_date,
			packaging_type: args.packaging_type || args.packing_type,
		});
		return;
	}
	...
	promises.push(
		needs_so
			? frappe.db.get_value('Sales Order', args.sales_order, ['customer', 'transaction_date'])
			: Promise.resolve(null)
	);
	promises.push(
		needs_soi
			? frappe.db.get_value('Sales Order Item', args.sales_order_item, ['qty', 'real_qty', 'picked_qty'])
			: Promise.resolve(null)
	);

	Promise.all(promises).then(([so_r, soi_r]) => {
		const enriched = {
			...args,
			customer: args.customer || args.frm?.doc?.customer,
			date: args.date || args.transaction_date,
		...
		if (so_r?.message) {
			enriched.customer = enriched.customer || so_r.message.customer;
			enriched.date = enriched.date || so_r.message.transaction_date;
			enriched.transaction_date = enriched.transaction_date || so_r.message.transaction_date;
		}

		if (soi_r?.message) {
			enriched.so_qty = enriched.so_qty || soi_r.message.qty;
			enriched.so_real_qty = enriched.so_real_qty || soi_r.message.real_qty;
			if (enriched.remaining_to_pick === undefined) {
				enriched.remaining_to_pick = flt(soi_r.message.qty) - flt(enriched.picked_qty || soi_r.message.picked_qty || 0);
			}
		}

		open_dialog(enriched);
	});
};

const view_pick_list_details = (args) => {
	frappe.require("assets/malaga/js/utils/view_pick_list_details.js", function () {
	...
	action: function (e) {
		...
		if (!cur_dialog) {
			...
			select_items({
				...
				remaining_to_pick: (d.qty - d.picked_qty - d.so_delivered_without_pick),
				batch_no: d.batch_no,
				qty: d.qty,
